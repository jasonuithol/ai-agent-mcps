...
echo "Connecting to dotnet-mcp-knowledge..."
K_SESSION=$(get_session "$BASE")
if [ -z "$K_SESSION" ]; then
    echo "ERROR: Could not get MCP session from dotnet-mcp-knowledge ($BASE)"
...
if [ -d "$HOME/Projects/betl" ]; then
    ...
    call_tool "$BASE" "$K_SESSION" 3 "seed_dotnet_source" \
